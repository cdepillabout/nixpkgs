...
let
  ...
  sources =
    if builtins.pathExists ./upstream-info.nix
  ...
  getDebURL = channelName: version: arch: mirror:
    let
...
rec {
      ...
      binary = fetchurl
        ( let
            mkUrls = arch:
              ...
              in
              map mkURLForMirror ([ debURL ] ++ debMirrors);
          in
          if stdenv.is64bit && chanAttrs ? sha256bin64 then
            {
              urls = mkUrls "amd64";
  ...
  */
  ...
  csv2nixInitial = csv2nix "omahaproxy-info" "http://omahaproxy.appspot.com/all?csv=1";
  ...
  isLatest = channel: version:
    let
      ourVersion = sources.${channel}.version or null;
    in
    if ourVersion == null then
      false
    else
      lib.versionOlder version sources.${channel}.version ||
  ...
  update =
    let
      ...
      genLatest = channelName: channel:
        ...
        in
        if isLatest channelName channel.version then keepOld else newUpstream;

      # We only support GNU/Linux right now.
      linuxChannels = lib.mapAttrs genLatest channels.linux;

      getLinuxFlash = channelName: channel:
        let
          inherit (channel) version;
          fetchArch = arch: tryFetch (getDebURL channelName version arch debURL);
          packages = lib.genAttrs ["i386" "amd64"] fetchArch;
          isNew = arch: attr: !(builtins.hasAttr attr channel) && packages.${arch}.success;
        in
        channel // lib.optionalAttrs (isNew "i386" "sha256bin32") {
      ...
      dumpAttrs = indent: attrs:
        let
          mkVal = val:
            if lib.isAttrs val then
              dumpAttrs (indent + 1) val
            else
              "\"${lib.escape ["$" "\\" "\""] (toString val)}\"";
    ...
    in
    writeText
      "chromium-new-upstream-info.nix"
      ''
